...
class Trainer:
    ...
    def build_data_loader(self):
        ...
        lb_class_dist = [0 for _ in range(cfg.DATA.NUMBER_CLASSES)]
        for c in labeled_dataset.targets:
            lb_class_dist[c] += 1
        lb_class_dist = np.array(lb_class_dist)
        self.lb_class_dist = torch.from_numpy(np.min(lb_class_dist) / lb_class_dist)
        print("Labeled data class distribution:", self.lb_class_dist)
    ...
    def compute_abc_loss(self, logits_x_lb, y_lb, logits_x_ulb_w, logits_x_ulb_s):
        ...
        if not self.lb_class_dist.is_cuda:
            self.lb_class_dist = self.lb_class_dist.to(self.device)

        # compute labeled abc loss
        mask_lb = self.lb_class_dist[y_lb]  # simplified, no bernouli
        ...
        with torch.no_grad():
            ...
            mask_ulb = max_probs.ge(self.abc_p_cutoff).to(logits_x_ulb_w.dtype)  # simplified, no ulb_class_dist
    ...
    def train(self):
        ...
        for self.epoch in range(self.start_epoch, self.num_epochs):
            ...
            for self.batch_idx in range(self.cfg.total_steps):
                ...
                output1_u_w_twice = torch.cat([output1_u_w, output1_u_w], dim=0).to(self.device)

                abc_loss = self.compute_abc_loss(
                            logits_x_lb=output1_x, 
                            y_lb=targets_x,
                            logits_x_ulb_w=output1_u_w_twice,
                            logits_x_ulb_s=output1_u_s_twice
                            )
                
                loss = Lx + Lu + abc_loss * self.abc_loss_ratio
