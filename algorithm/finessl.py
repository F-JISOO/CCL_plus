...
class Trainer:
    ...
    @torch.no_grad()
    def test_and_save_confusion(self, save_pt_path="test_pred_and_cm.pt", save_conf=True):
        """
        在 test_loader 上推理，收集 y_true / y_pred（可选 conf），并保存 confusion matrix(cm)。
        保存内容：
          - acc
          - num_classes
          - total_samples
          - y_true: [N]
          - y_pred: [N]
          - conf : [N] (可选，pred 对应的最大 softmax 置信度)
          - cm   : [C, C]，cm[i,j]=count(true=i, pred=j)
        """
        self.model.eval()
        self.tuner.eval()
        print("Evaluate on the test set (and collect y_true/y_pred + confusion matrix)")

        num_classes = int(self.num_classes)

        y_true_list = []
        y_pred_list = []
        conf_list = [] if save_conf else None
        ...
        for batch in tqdm(self.test_loader, ascii=True):
            ...
            conf, pred = prob.max(dim=1)  # [B], [B]

            # 收集到 CPU（避免 GPU 累积占显存）
            y_true_list.append(label.detach().cpu().long())
            y_pred_list.append(pred.detach().cpu().long())
            if save_conf:
                conf_list.append(conf.detach().cpu().float())

        y_true = torch.cat(y_true_list, dim=0)  # [N]
        y_pred = torch.cat(y_pred_list, dim=0)  # [N]
        total = int(y_true.numel())

        # acc
        acc = (y_true == y_pred).float().mean().item()

        # confusion matrix: cm[i,j] = count(true=i, pred=j)
        ...
        if save_conf:
            pack["conf"] = torch.cat(conf_list, dim=0)  # [N]

        torch.save(pack, save_pt_path)
        print(f"[Saved] test preds + cm -> {save_pt_path}")
    ...
    def train(self):
        ...
        pack = {
            ...
            "method": "finessl",  # 你也可以写 cfg.method
        }
    ...
    def train_black_model(self):
        ...
        for self.epoch in range(self.start_epoch, self.num_epochs):
            ...
            best_acc = max(best_acc, acc_now)
            self.logger.append([acc_now, best_acc, self.epoch + 1, avg_time_cost])
    ...
    def offline_ensemble_test(self):
        ...
        state_dict1 = self.load_model_stat('out/semi_aves/finessl_single_linear_openclip/Semi_Aves/NUML50_imbl1.0_imbu1.0/epoch_30', epoch=30)
        ...
        state_dict = self.load_model_stat('out/semi_aves/finessl_single_clip/Semi_Aves/NUML50_imbl1.0_imbu1.0/epoch_30', epoch=30)
